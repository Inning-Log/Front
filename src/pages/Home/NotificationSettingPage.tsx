...
export function NotificationSettingPage() {
  ...
  useEffect(() => {
    const fetchNotificationSettings = async () => {
      try {
        ...
        setGameNotification(data.gameProgressEnabled);
        setRecordNotification(data.recordReminderEnabled);
        setReactionNotification(data.socialReactionEnabled);
      } catch (error) {
  ...
  const handleGameNotificationToggle = async () => {
    ...
    try {
      ...
      console.error("경기 진행 상황 알림 설정 변경 실패:", error);
    }
  };

  const handleRecordNotificationToggle = async () => {
    ...
    try {
      ...
      console.error("기록 독촉 알림 설정 변경 실패:", error);
    }
  };

  const handleReactionNotificationToggle = async () => {
    ...
    try {
      ...
      console.error("댓글 또는 반응 알림 설정 변경 실패:", error);
    }
  };
    ...
    <main className="min-h-dvh w-full bg-bg-primary pt-[45px]">
      ...
      <section className="mx-auto mt-[25px] w-full max-w-[340px]">
        <NotificationSettingItem
          ...
          onToggle={handleGameNotificationToggle}
        />
        ...
        <NotificationSettingItem
          ...
          onToggle={handleRecordNotificationToggle}
        />
        ...
        <NotificationSettingItem
          ...
          onToggle={handleReactionNotificationToggle}
        />
      </section>
    </main>
  );
